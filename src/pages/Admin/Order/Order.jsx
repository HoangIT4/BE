...
const Order = () => {
  ...
  const [selectedOrder, setSelectedOrder] = useState(null); // State lưu đơn hàng được chọn
  ...
  const handleUpdateStatus = async (orderID, newStatus) => {
    try {
      ...
      if (response.status) {
        ...
        // Nếu cập nhật thất bại, giữ nguyên giá trị cũ
  ...
  const handleDelete = async () => {
    try {
      ...
      const updatedOrders = orders.filter(
        (_, index) => index !== selectedOrderIndex
      );
          ...
          <tbody>
            {orders && orders.length > 0 ? (
              orders.map((order, index) => (
                <tr 
                  key={order.orderID}
                  onClick={() => handleShowDetailModal(order)} // Mở Modal chi tiết khi click
                  onContextMenu={(e) => handleRightClick(e, index)} // Xử lý nhấp chuột phải
                >
                  <td>{order.orderID}</td>
                  <td>{order.userID}</td>
                  <td>{order.address}</td>
                    ...
                    {updatingOrderID === order.orderID ? (
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          handleUpdateStatus(order.orderID, newStatus);
                        }}
                        disabled={updatingOrderID !== null} // Disable trong khi đang cập nhật
                      >
